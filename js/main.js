...
function onDeviceReady(){
	...
	db = window.openDatabase("agenda_curso", "1.0", "DB de la agenda", 2000000);
	if(existe_db == null){
		creaDB();
	}else{
		cargaDatos();
...
function queryDetalleSuccess(tx, results) {
	...
	$("#categoria").html($.registro.categoria);
		...
		$("#label_peticion").html("Peticion: " + $.registro.peticion);
		$("#label_ciudad").html("Ciudad: " + $.registro.ciudad);
		$("#label_direccion").html("Dirección: " + $.registro.direccion);
		$("#label_genero").html("Género: " + $.registro.genero);
}
...
function queryFormSuccess(tx, results) {
	...
	$.registro = results.rows.item(0);
		...
		$("#ti_peticion").val($.registro.peticion);
		$("#ti_ciudad").val($.registro.ciudad);
		$("#ti_direccion").val($.registro.direccion);
		$("#ti_genero").val($.registro.genero);
...
function queryDBUpdateForm(tx){
	...
	tx.executeSql('UPDATE agenda_curso SET nombre="'+$("#ti_nombre").val()+'", apellidos="'+$("#ti_apellidos").val()+'",telefono="'+$("#ti_telefono").val()+'",email="'+$("#ti_mail").val()+'",categoria="'+cat+'",foto = "'+$.imageURL+'", peticion ="'+$("#ti_peticion").val()+'", ciudad ="'+$("#ti_ciudad").val()+'", direccion ="'+$("#ti_direccion").val()+'", genero ="'+$("#ti_genero").val()+'" WHERE id='+$.id);
}
...
function queryDBInsertForm(tx){
	...
	tx.executeSql("INSERT INTO agenda_curso (nombre,apellidos,telefono,categoria,foto,email,peticion,ciudad,direccion,genero) VALUES ('"+$("#ti_nombre").val()+"','"+$("#ti_apellidos").val()+"','"+$("#ti_telefono").val()+"','"+cat+"','"+$.imageURL+"','"+$("#ti_mail").val()+"','"+$("#ti_peticion").val()+"','"+$("#ti_ciudad").val()+"','"+$("#ti_direccion").val()+"','"+$("#ti_genero").val()+"')", [], newFormSuccess, errorDB);
}
